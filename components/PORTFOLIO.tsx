...
export default function PORTFOLIO() {
    ...
    return (
        <div className='relative w-full h-full flex flex-col gap-[1vw] xl:gap-[1.5vw]'>
            ...
            <div className="w-full flex gap-[1.5vw] p-[1.5vw] pt-2">
                {projects.map((project, index) => (
                    <div key={index} className="group flex flex-col gap-[1vh] items-center">
                        ...
                        <Link href={project.url} target="_blank">
                            <h3 className='group-hover:text-red-500 text-[1vw] text-white text-center quicksand font-bold cursor-pointer duration-300'>{project.name}</h3>
                        </Link>
                    </div>
                ))}
            </div>
